    ...
    <div id="projects" className="container py-24 sm:py-32">
      ...
      <div className="mt-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {projects.map((project) => {
          let actualImageUrl = '';
          let imageHint = 'project screenshot';

          if (project.imageUrl?.startsWith('http')) {
            actualImageUrl = project.imageUrl;
          } else if (project.imageUrl) {
            const imgObj = PlaceHolderImages.find((p) => p.id === project.imageUrl);
            if (imgObj) {
              actualImageUrl = imgObj.imageUrl;
              imageHint = imgObj.imageHint || imageHint;
            }
          }
